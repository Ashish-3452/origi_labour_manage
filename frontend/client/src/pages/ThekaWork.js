...
const ThekaWork = () => {
  ...
  const [form, setForm] = useState({
    ...
    work_type: 'THEKA',
    company_hajri: 3, company_rate: 500,
    labour_hajri_per_person: 2, labour_rate: 400, num_labours: 5,
    mason_hajri_per_person: 3, mason_rate: 650, num_masons: 1
  });
  ...
  const handleCalculate = async () => {
    ...
    try {
      const res = await api.post('/theka/calculate', {
        ...
        mason_hajri_per_person: form.mason_hajri_per_person,
        mason_rate: form.mason_rate,
        num_masons: form.num_masons
      });
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/theka/save', {
        ...
        total_labour_hajri: (form.labour_hajri_per_person * form.num_labours),
        labour_payment: result.labour_payment,
        total_mason_hajri: (form.mason_hajri_per_person * form.num_masons),
        mason_payment: result.mason_payment,
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 }, bgcolor: '#f5f5f5', minHeight: '100vh' }}>
      <Typography variant="h5" fontWeight="bold" mb={3}>🎯 Theka / Contract Work (with Mason)</Typography>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={5}>
          <Paper sx={{ p: 3, borderRadius: 3 }}>
            ...
            <Typography variant="subtitle1" fontWeight="bold" mt={2}>👷‍♂️ Mason (Mistri) Side</Typography>
            <Grid container spacing={1}>
              <Grid item xs={4}>
                <TextField label="Hajri/Mason" type="number" value={form.mason_hajri_per_person}
                  onChange={(e) => setForm({...form, mason_hajri_per_person: e.target.value})} />
              </Grid>
              <Grid item xs={4}>
                <TextField label="Rate" type="number" value={form.mason_rate}
                  onChange={(e) => setForm({...form, mason_rate: e.target.value})} />
              </Grid>
              <Grid item xs={4}>
                <TextField label="Masons" type="number" value={form.num_masons}
                  onChange={(e) => setForm({...form, num_masons: e.target.value})} />
              </Grid>
            </Grid>

            <Button variant="contained" fullWidth startIcon={<Calculate />}
              onClick={handleCalculate} sx={{ mt: 3, py: 1.5 }}>
              Calculate Profit
            </Button>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={7}>
            ...
            <Card sx={{ mb: 3, bgcolor: result.is_profitable ? '#e8f5e9' : '#ffebee' }}>
              <CardContent>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6}><Typography>Mason Payment</Typography><Typography variant="h6">₹{result.mason_payment.toLocaleString()}</Typography></Grid>
                  <Grid item xs={6}><Typography>Total Labour Cost</Typography><Typography variant="h6">₹{result.total_labour_cost.toLocaleString()}</Typography></Grid>
                  <Grid item xs={12}>
          ...
          <Paper sx={{ p: 3, borderRadius: 3 }}>
            ...
            <TableContainer sx={{ overflowX: 'auto' }}>
              <Table size="small">
                <TableHead>
                  <TableRow><TableCell>Date</TableCell><TableCell>Site</TableCell><TableCell>Company</TableCell><TableCell>Labour</TableCell><TableCell>Mason</TableCell><TableCell>Profit</TableCell></TableRow>
                </TableHead>
                <TableBody>
                  {history.map(h => (
                    <TableRow key={h.id}>
                      ...
                      <TableCell>₹{Number(h.total_mason_payment || 0).toLocaleString()}</TableCell>
                      <TableCell>
